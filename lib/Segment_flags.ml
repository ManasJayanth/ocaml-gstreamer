...
type t = None | Reset | Trickmode | Skip | Segment | Trickmode_key_units | Trickmode_no_audio
type t_list = t list

let of_value v =
  if v = Unsigned.UInt32.of_int 0 then None
  else if v = Unsigned.UInt32.of_int 1 then Reset
  ...
  else raise (Invalid_argument "Unexpected Segment_flags value")

let to_value = function
  | None -> Unsigned.UInt32.of_int 0
  | Reset -> Unsigned.UInt32.of_int 1
...
let list_of_value v =
  ...
  let all_flags = [( 0 , None ); ( 1 , Reset ); ( 16 , Trickmode ); ( 16 , Skip ); ( 8 , Segment ); ( 128 , Trickmode_key_units ); ( 256 , Trickmode_no_audio )]
  in
